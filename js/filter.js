...
// 判断是现金还是积分
 ...
 //隐藏旺旺名字
  m1.filter('ww_hide', ['$sce',function($sce) {
   return function(r) {  
      if(!r)  return '';
      return  r.slice(0,1) + '*****' + r.charAt(r.length - 1);
    }
  }])

//隐藏银行卡中间部分
m1.filter('bank_hide', ['$sce',function($sce) {
 return function(r) {  
    if(!r)  return '';
...
//提现手续费计算
m1.filter('f_money_fee', ['$sce',function($sce) {
 return function(r) {
      if(!r) return '';
      var fee =  Math.ceil(r / 100 * $('#service_Counter_Fee').text());
      var text = '';
      if(!fee) return '免手续费';
      if(fee < $('#service_min_counter_fee').text()){
         //手续费小于当前设置的手续费
         var t_moeny =  r - $('#service_min_counter_fee').text();
         return text +=  '手续费'+$('#service_min_counter_fee').text() +'元,最终到账：'+t_moeny+ '元';
      }else{
         var t_moeny =  r - fee;
         return text += '手续费' + fee + '元,最终到账：'+t_moeny+ '元';
      }
  }
}])
 ...
 m1.filter('coverhtml', ['$sce',function($sce) {
...
m1.filter('pay_fee', ['$sce',function($sce) {
    return function(r) {
        if(!r) return '';
        r = parseFloat(r);
...
m1.filter('dateLocal', ['$sce',function($sce) {
	return function(r) {
	    var time_str = '' + r;
		if(time_str.length < 13) {
            time_str += '000';
        }
        var date = new Date(time_str-0);
