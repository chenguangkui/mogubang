...
/***/ function(module, exports) {
	...
	var loader = exports.loader = function loader(callback) {
		...
		scriptHeat.id = 'captcha_script';
	    scriptHeat.onload = onload;
	    var captcha_script = $('#captcha_script');
	    if (captcha_script.length) {
		    captcha_script.remove();
	    }
		body.appendChild(scriptHeat);

	    var scriptResponse = document.createElement('script');
	    scriptResponse.type = 'text/javascript';
	    scriptResponse.text = 'function luosimaoCaptchaResponse(response) { \
	    };';
		scriptResponse.id = 'captcha_response';
		var captcha_response = $('#captcha_response');
	    if (!captcha_response.length) {
		    body.appendChild(scriptResponse);
	    }
	};
